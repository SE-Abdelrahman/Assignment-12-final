...
export class Details implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      this.article = this.allArticles.find((a) => a.slug === slug);
    });
  }

  getParsedContent() {
    if (!this.article) return [];
    const sections = this.article.content.split('##');
    return sections.map((section, index) => {
      const lines = section.trim().split('\n');
      return {
        title: index === 0 ? '' : lines[0].trim(),
        text: index === 0 ? lines.join('\n') : lines.slice(1).join('\n'),
      };
    });
  }

  getRelatedArticles(): AllArticles[] {
    if (!this.article) return [];

...
import { AllArticles } from '../../interfaces/all-articles';
...
export class Blog implements OnInit {
  ...
  applyFilters() {
    const baseFiltered = this.allArticles.filter((article) => {
      const matchesCategory =
        this.selectedCategory === 'جميع المقالات' ||
        article.category.trim() === this.selectedCategory.trim();
    ...
    this.resultsCount = baseFiltered.length;

    this.totalPages = Math.ceil(this.resultsCount / this.itemsPerPage);

    const startIndex = (this.currentPage - 1) * this.itemsPerPage;
    this.filteredArticles = baseFiltered.slice(startIndex, startIndex + this.itemsPerPage);
  }
